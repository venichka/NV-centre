...
s1_oper, s2_oper = comp_cl.opers_inbasis(M_coupling=g0[0], NV_coupling=g0[1])

'''Transition matrix'''

gamij1 = np.zeros([2, 2*N_mode, 2*N_mode])
gamij = np.zeros([2, 2*N_mode, 2*N_mode])
#gamij2 = np.zeros([2, 2*N_mode, 2*N_mode])
gamij1[0] = g0[0] * np.abs(np.dot(s1_oper, np.conjugate(np.transpose(s1_oper))))
gamij1[1] = g0[1] * np.abs(np.dot(s2_oper, np.conjugate(np.transpose(s2_oper))))
gamij[0] = g0[0] * np.abs(np.transpose(s1_oper))**2
gamij[1] = g0[1] * np.abs(np.transpose(s2_oper))**2
#gam_up = np.exp(-)
gam_up = np.array([[gamij[0, i, j] * \
                                 np.exp(- (w_values[i] - w_values[j]) / T) \
                                 if i < j else \
                                 0 for i in range(len(w_values))] \
                                for j in range(len(w_values))])
gamij[0] = gamij[0] + gam_up
