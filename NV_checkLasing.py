...
g0 = np.array([0.04, 0.1, 0.15])  #- gammas for S1, S2, S3
...
s1_oper, s2_oper, s3_oper = comp_cl.opers_inbasis(M_coupling=0., \
                                                  NV_coupling=g_s1[1])
...
for i in range(nStep):
    ...
    numb_phot[i] = np.trace(np.dot(np.diag(pdiag), aca_oper))
