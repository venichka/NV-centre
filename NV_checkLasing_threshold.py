...
N_mode = 100
h_planck = 1.
T = 0.0000000004  #temperature of radiation resevoir (S1)
         #[0.4 coincides with 300K if h_plank = 1.]
T_ph = 0.00000000002  #temperature of dephasing reservoir (S2)
T_pump = - 23.96  #negative temperature of pumping (S3)
omR = 0.1
omM = 1.
omNV = 1.
delt = omM - omNV
g_s1 = np.array([0.001, 0.09])  #- gammas for S1 (g_s1[0] * a + g_s1[1] * sigma)
g0 = np.array([1.04, 0.1, 0.15])  #- gammas for S1, S2, S3
g_pmax = 0.021  #- max pumping constant
...
s1_oper, s2_oper, s3_oper = comp_cl.opers_inbasis(M_coupling=g_s1[0], \
                                                  NV_coupling=g_s1[1])
a_oper, ac_oper = comp_cl.mode_opers()
aca_oper = np.dot(ac_oper, a_oper)
aca_oper_inbasis = np.dot(np.transpose(v_vectors), np.dot(aca_oper, v_vectors))

"""Variables for solution"""

pdiagT = np.zeros([len(v_vectors[:, 0]), nT])  #- Probabilities
DHamAv = np.zeros(nStep)  #- Derivative of hamiltonian
numb_phot = np.zeros(nStep)  #number of photons in the system
numb_phot_st = np.zeros(n_cycle)

for l in range(n_cycle):

    '''Change pumping'''

    g0[2] = g_pmax * l / (n_cycle - 1.)
    ...
    #For lasing only
    gam_up0 = np.array([[gamij[2, i, j] \
                                     if i < j else \
                                     0 for i in range(len(w_values))] \
                                    for j in range(len(w_values))])
    gamij[2] = gam_up0

    print(g0[2], np.max(gamij[0]), np.max(gamij[1]), np.max(gamij[2]))
    """Solve Lindblad equation"""


    #pdiag = incon(numState) #- initial conditions
    pdiag = incon_c(2 * N_mode - 1)

    for i in range(nStep):
        #print(i)
        pdiag = scheme(pdiag, gamij, alpha=0.5, beta=0.5)
        DHamAv[i] = - np.dot(detpart(pdiag, gamij), w_values)
        numb_phot[i] = np.trace(np.dot(np.diag(pdiag), aca_oper_inbasis))
        #--- Probabilities in every 20 step
        if np.mod(i, dsT) == 0:
            pdiagT[:, np.int(i/dsT)] = pdiag
    numb_phot_st[l] = np.trace(np.dot(np.diag(pdiag), aca_oper_inbasis))
...
ax2.plot(numb_phot, 'r')
ax2.set_title("Number of photons, N_mode = %s" % N_mode)
...
ax11.plot(numb_phot_st, 'b')
ax11.set_title("Number of photons vs. pumping, N_mode = %s" % N_mode)
